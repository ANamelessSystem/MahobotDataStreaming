...
namespace Marchen.BLL
{
    class CaseRemind : GroupMsgBLL
    {
        ...
        public static void NoticeRemainStrikers(string strGrpID, string strUserID, GroupMemberInfo memberInfo)
        {
            ...
            try
            {
                RecordDAL.QueryStrikeStatus(strGrpID, out dtResult);
            }
            catch (Exception ex)
            {
            ...
            {
                string strUID = dtResult.Rows[i]["MBRID"].ToString();
                int intCountMain = int.Parse(dtResult.Rows[i]["cmain"].ToString());
                int intCountLastAtk = int.Parse(dtResult.Rows[i]["cla"].ToString());
                int intCountExTime = int.Parse(dtResult.Rows[i]["cex"].ToString());
        ...
        public static void ShowRemainStrikes(string strGrpID, string strUserID,string strCmdContext)
        {
            ...
            {
                string strUID = dtResult.Rows[i]["MBRID"].ToString();
                string strUName = dtResult.Rows[i]["MBRNAME"].ToString();
                int intCountMain = int.Parse(dtResult.Rows[i]["cmain"].ToString());
                int intCountLastAtk = int.Parse(dtResult.Rows[i]["cla"].ToString());
                int intCountExTime = int.Parse(dtResult.Rows[i]["cex"].ToString());
                if ((intCountMain + intCountLastAtk) > 3)
